 ...
 * @author yanga
 */
public class EtudiantParClasse extends javax.swing.JFrame {

    /**
     * Creates new form EtudiantParClasse
     */
    public EtudiantParClasse() {
        initComponents();
    ...
    private void AfficherEtudiant() {
      ...
      List<Scolarite> list = ScolariteDao.getScolariteDao().findScolariteEntities();
      
      Pscolarite sc = new Pscolarite();
      List<Pscolarite> Lsc = new ArrayList<>();
              
      for(Scolarite s : list){
          sc.setClasse(s.getClasseId().getNom());
          sc.setEtudiant(s.getEtudiantId().getNom() + " - " + s.getEtudiantId().getPrenom());
          sc.setMontant_Payer(s.getMtnP());
          sc.setReste_A_Payer(s.getMtnR());
          
          Lsc.add(sc);
      }
      
      UtilsFonction.displayDataInTable(Lsc, tableEtudiant);
    }
    
    
 private void AfficherEtudiantByClasse(String classe) {
    List<Scolarite> list = ScolariteDao.getScolariteDao().findScolariteEntities();
    
    List<Scolarite> sco = list.stream()
        .filter(sc -> sc.getClasseId() != null && sc.getClasseId().getNom().equals(classe))
    ...
    if (!sco.isEmpty()) {
        UtilsFonction.displayDataInTable(Lsc, tableEtudiant, List.of("id"));
    } else {
        JOptionPane.showMessageDialog(this, "Classe est vide !!!");
        AfficherEtudiant();
        rClasse_txt.setText("");
    }
}
    ...
    private void initComponents() {
        ...
        welcomeText1.setText("Liste des eleves par Classe");
    ...
    private void scolarite_btn1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_scolarite_btn1ActionPerformed
      ...
      if(!recherche.isBlank()){
          AfficherEtudiantByClasse(recherche);
      }else{
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(EtudiantParClasse.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
